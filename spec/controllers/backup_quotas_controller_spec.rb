...
context BackupQuotasController do
  ...
  context "ssh_key_send" do
    ...
    specify "should execute shell command to submit key to console.cust (without append)" do
      # expect(Kernel).to receive(:system).with("ssh -o 'ConnectTimeout 5' locksmith@#{@server} add 0 #{@login} #{@quota} '#{@key}'")
      expect(Kernel).to receive(:system).\
        with("ssh", "-o", "ConnectTimeout 5", "#{$KEYER}@#{@server}", "add", 0, @login, @quota, @key)
    ...
    specify "should execute shell command to submit key to console.cust (with append)" do
      # expect(Kernel).to receive(:system).with("ssh -o 'ConnectTimeout 5' locksmith@#{@server} add 1 #{@login} #{@quota} '#{@key}'")
      expect(Kernel).to receive(:system).\
        with("ssh", "-o", "ConnectTimeout 5", "#{$KEYER}@#{@server}", "add", 1, @login, @quota, @key)
