class Admin::ReportsController < Admin::HeadQuartersController
  ...
  def services
    vps_service_code = ServiceCode.find_by(name: 'VPS')
    ip_block_service_code = ServiceCode.find_by(name: 'IP_BLOCK')
    metal_service_code = ServiceCode.find_by(name: 'METAL')
    backup_service_code = ServiceCode.find_by(name: 'BACKUP')
    thunder_service_code = ServiceCode.find_by(name: 'THUNDER')
  ...
  def check_access
    redirect_to admin_path unless @is_super_admin
  end
end
